...
Now, based on the case study concept, we present a solution architecture. Firstly, our Topology of TAG consists of three worker nodes, one for the United States, Europe, and Asia. In a real situation, the worker nodes will get the models, data and so on from other smaller resources like single software houses, but to make this example more general we skip the lowest level of the hierarchy. Then, after we compute models in worker nodes, we merge models together in the master node into one bigger model that aggregates results from worker nodes. Those nodes will be simulated using Kubernetes. Kubernetes is an open-source container orchestration platform that automates the deployment, scaling, and management of containerized applications. It provides a consistent way to deploy and manage applications across different environments, such as on-premises data centers and public clouds. Kubernetes also allows for easy scaling and resiliency of applications, making it a popular choice for modern cloud-native architectures. We can run our architecture using cloud computing, but we decided that we will run it locally. Apart from that, we will present yet another architecture. The previous one presents the aspects of federated learning. In the second one, we will introduce a schema for distributed learning. The difference between the previous example is that our working nodes are connected with each other. There is no one, centralized master node that manages worker nodes. They rather communicate with each other and use their sub-results. For this example, we will also use local Kubernetes.
